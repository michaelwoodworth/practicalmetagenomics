...
script_path=                  # you can specify the path where you have saved these scripts if not in path
```
...
for ID in `cat ${IDlist}`; do echo $ID; python ${script_path}/00_amrfinder_filter.py -i ${indir}/${ID}_amrfinder.tsv -o ${outdir}/${ID} -m add_partial_end -j -v; done
```
