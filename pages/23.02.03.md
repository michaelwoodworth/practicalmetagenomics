...
# Set path of index as ${index} if run while in directory where GRCh38 was downloaded
index=`realpath GRCh38_noalt_as`
```

- [OPTION] Create bowtie2 index for other reference genome to remove
```console
bowtie2-build ${path_to_host_reference_genome} ${path/name_of_bowtie2_index}
```
